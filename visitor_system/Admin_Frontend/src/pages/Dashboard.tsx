...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        const [resHeat, resSugg, resFin, resEvents, resFore, resProf] = await Promise.all([
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                fontWeight: 'bold',
                color: 'white'
              }}>{adminProfile?.initials ?? "DS"}</div>
              <div>
                <div style={{ fontSize: '14px', fontWeight: '600', color: '#0D2B1A' }}>{adminProfile?.name ?? "System Admin"}</div>
                <div style={{ fontSize: '12px', color: '#8B5E3C' }}>{adminProfile?.role ?? "Administrator"}</div>
              </div>
            </div>
      ...
      <main style={{ maxWidth: '1400px', margin: '0 auto', padding: '40px 30px' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '40px', fontWeight: 'bold', color: '#0D2B1A', marginBottom: '8px' }}>{finance?.ai_accuracy ?? 0}%</div>
